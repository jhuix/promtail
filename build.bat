...
rem set DYN_GO_FLAGS=-ldflags "-s -w %GO_LDFLAGS%" -tags netgo
set DEBUG_GO_FLAGS=-gcflags "all=-N -l" -ldflags "-extldflags \"-static\" %GO_LDFLAGS%" -tags netgo
rem set DYN_DEBUG_GO_FLAGS=-gcflags "all=-N -l" -ldflags "%GO_LDFLAGS%" -tags netgo

rem set PROMTAIL_GENERATED_FILE=pkg/promtail/server/ui/assets_vfsdata.go
