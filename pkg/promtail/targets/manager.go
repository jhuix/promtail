...
import (
	...
	"github.com/grafana/loki/pkg/promtail/targets/forward"
...
const (
	...
	ForwardScrapeConfigs = "ForwardScrapeConfigs"
)
...
func NewTargetManagers(
	...
	for _, cfg := range scrapeConfigs {
		...
		case cfg.ForwardConfig != nil:
			targetScrapeConfigs[ForwardScrapeConfigs] = append(targetScrapeConfigs[ForwardScrapeConfigs], cfg)
		default:
	...
	for target, scrapeConfigs := range targetScrapeConfigs {
		...
		case ForwardScrapeConfigs:
			forwardTargetManager, err := forward.NewForwardTargetManager(
				logger,
				client,
				scrapeConfigs,
			)
			if err != nil {
				return nil, errors.Wrap(err, "failed to make forward target manager")
			}
			targetManagers = append(targetManagers, forwardTargetManager)
		default:
			return nil, errors.New("unknown scrape config")
		}
	}
