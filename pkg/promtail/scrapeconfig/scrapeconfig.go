...
type Config struct {
	...
	ForwardConfig          *ForwardTargetConfig   `yaml:"forward,omitempty"`
...
// ForwardTargetConfig describes a scrape config that listens for Loki forward messages.
type ForwardTargetConfig struct {
	// Server is the forward client config for forward connections
	Client forwarder.ClientConfig `yaml:"client"`
